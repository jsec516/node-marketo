var _ = require('lodash'),
    Promise = require('bluebird'),
    Readable = require('stream').Readable,
    util = require('util'),
    log = require('./util').logger(),
    errors = require('./errors');

function MarketoStream(marketoResultPromise, options) {
  options = _.defaults(options || {}, {objectMode: true});
  Readable.call(this, options);
  this._setData(marketoResultPromise || Promise.resolve({}));
  this.cacheOptions = options;
  this.marketoIns = arguments[2];
  this.reqListId = arguments[3];
  this.numOfFailedTrials = 0;
  log.debug('additional information status ', {cacheOptions: this.cacheOptions, marketoIns: this.marketoIns, reqListId: this.reqListId});
}

util.inherits(MarketoStream, Readable);

/* edit for failover */
MarketoStream.prototype._retry = function(marketoObj, options, reqListId) {
  var self = this;
  var resumeOptions = {nextPageToken: self.nextPageToken};
  var marketoResultPromise = marketoObj.list.getLeads(reqListId, resumeOptions);
  options = _.defaults(options || {}, {objectMode: true});
  Readable.call(this, options);
  this._setData(marketoResultPromise || Promise.resolve({}));
};

MarketoStream.prototype._setData = function(dataPromise) {
  this._ready = false;

  var self = this;
  this._dataPromise = dataPromise;
  return dataPromise.then(
    function(data) {
      self.numOfFailedTrials = 0;
      if (data.result) {
        console.log('we are in here');
        self._data = data;
        self.nextPageToken = data.nextPageToken;
        self._resultIndex = 0;
        self._ready = true;
        self._pushNext();
      } else {
        self.push(null);
      }
    },
    function(err) {
      if (err.errors && err.errors[0].code == '605') {
        if (self.numOfFailedTrials < 3) {
          self.numOfFailedTrials++;
          self._retry(self.marketoIns, self.cacheOptions, self.reqListId);
        } else {
          self.numOfFailedTrials = 0;
          var message = err.errors[0].message;
          var customError = new errors.HitLimitExceedError(605, message, self.nextPageToken);
          self.emit('error', customError);
          // end the stream
          self.push(null);
        }
      } else {
        self.emit('error', err);
        // end the stream
        self.push(null);
      }
      
    });
};

MarketoStream.prototype._read = function() {

  if (!this._ready) {
    return;
  }

  if (this._data.result) {
    if (this._pushNext()) {
      return;
    } else if (this._data.nextPageToken) {
      this._setData(this._data.nextPage())
    } else {
      // No data left and no more data from marketo
      this.push(null);
    }
  } else {
    this.push(null);
  }
};

MarketoStream.prototype._pushNext = function() {
  var result;
  if (this._data.result) {
    result = this._data.result;
    if (this._resultIndex < result.length) {
      this.push(result[this._resultIndex++]);
      return true;
    }
  }

  return false;
};

MarketoStream.prototype.endMarketoStream = function() {
  this.push(null);
}

module.exports = MarketoStream;
