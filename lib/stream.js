...
function MarketoStream(marketoResultPromise, options) {
  ...
  this.tokenStream = new Readable();
  console.log('token stream instatiated', this.tokenStream);
  ...
  return dataPromise.then(
    function(data) {
      if (data.nextPageToken) {
        self.tokenStream.push(data.nextPageToken);
      }
      if (data.result) {
        self._data = data;
        self._resultIndex = 0;
...
MarketoStream.prototype._pushNext = function() {
  ...
  if (this._data.result) {
    ...
    if (this._resultIndex < result.length) {
      this.push({lead: result[this._resultIndex++], nextPageToken: this._data.nextPageToken});
      return true;
    }
  }
