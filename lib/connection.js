...
// logger added
console.log('some log should be made with winston');
winston.configure({
  transports: [
    new (winston.transports.File)({ filename: 'request-history.log' })
  ]
});
// logger end
  ...
  var requestFn = function requestFn(forceOAuth) {
    return this.getOAuthToken(forceOAuth)
      .then(function(token) {
        ...
        winston.log('debug', 'request method is ' + method + ' with  ', args);
        rest[method].apply(rest, args)
          .on('success', function(data, resp) {
            winston.log('debug', 'success response is ', {data: data, resp: resp});
          ...
          .on('error', function(err, resp) {
            winston.log('error', 'error happened ', {data: data, resp: resp});
            defer.reject(err);
          })
          .on('fail', function(data, resp) {
            winston.log('error', 'failure error happened ', {data: data, resp: resp});
