...
Connection.prototype._request = function(method) {
  ...
  if (global && global.apiNotifier && global.apiNotifier.notify) {
    apiNotifier.notify({
      orgId: global.mktoNotifyOpts && global.mktoNotifyOpts.orgId,
      orgName: global.mktoNotifyOpts && global.mktoNotifyOpts.orgName,
      integration: 'marketo',
      service: 'api',
      task: global.mktoNotifyOpts && global.mktoNotifyOpts.task,
      usageCount: 1,
      meta: {},
      endpoint: args[0]
    })
  } else {
    console.log('mkto call for unknown -> ' + args[0])
  }
    ...
    requestFn = function() {
      ...
      if (global && global.apiNotifier && global.apiNotifier.notify) {
        apiNotifier.notify({
          ...
          usageCount: 1,
          meta: {},
          endpoint: this._options.identity + OAUTH_TOKEN_PATH
        })
      } else {
        console.log('mkto call for unknown -> ' + this._options.identity + OAUTH_TOKEN_PATH)
      }
