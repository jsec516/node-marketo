...
Connection.prototype._request = function(method) {
  ...
  if (global && global.mktoLogger) {
    mktoLogger('mkto call for ' + mktoOrgName + ' -> ' + args[0], global.mktoReqInfo)
  } else {
...
Connection.prototype.getOAuthToken = function(force) {
  ...
  if (force || this._tokenData == null) {
    ...
    requestFn = function() {
      ...
      if (global && global.mktoLogger) {
        mktoLogger('mkto call for ' + mktoOrgName + ' -> ' + this._options.identity + OAUTH_TOKEN_PATH, global.mktoReqInfo)
      } else {
